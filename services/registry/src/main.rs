...
use ubl_model::{upsert_receipt, ReceiptNew};
use uuid::Uuid;
...
async fn main() -> anyhow::Result<()> {
    ...
    let cdn_base =
        std::env::var("CDN_BASE").unwrap_or_else(|_| "https://passports.ubl.agency".into());
    ...
    let app = Router::new()
        ...
        .route(
            "/v1/:app/:tenant/receipts/by-cid/:cid",
            get(receipts_get_by_cid),
        )
...
async fn receipts_post(
    ...
    auth.require_any_role(&["signer", "tenant_admin", "app_owner"])
    ...
    let tenant_id: Uuid =
        sqlx::query_scalar("SELECT id FROM tenant WHERE app_id = $1 AND slug = $2")
            .bind(app_id)
            .bind(&tenant)
            .fetch_optional(&st.pool)
            .await
            ...
            .ok_or((
                axum::http::StatusCode::NOT_FOUND,
                format!("tenant '{app}/{tenant}' not found"),
            ))?;

    // Enforce RBAC via membership (if user present)
    if let Some(user_str) = auth.user_id.as_ref() {
        // allow UUIDv4/7, else reject
        let user_id = Uuid::parse_str(user_str).map_err(|_| {
            (
                axum::http::StatusCode::UNAUTHORIZED,
                "x-user-id must be UUID".into(),
            )
        })?;
        let role: Option<String> =
            sqlx::query_scalar("SELECT role FROM membership WHERE user_id = $1 AND tenant_id = $2")
                .bind(user_id)
                .bind(tenant_id)
                .fetch_optional(&st.pool)
                .await
        ...
        let role_ok = role
            .as_deref()
            .map(|r| matches!(r, "signer" | "tenant_admin" | "app_owner"))
            .unwrap_or(false);
        if !role_ok {
            return Err((
                axum::http::StatusCode::FORBIDDEN,
                "membership role insufficient".into(),
            ));
        }
    } else {
        return Err((
            axum::http::StatusCode::UNAUTHORIZED,
            "missing x-user-id".into(),
        ));
    }
    ...
    let url = format!(
        "{}/{}/{}/receipts/{}.json",
        st.cdn_base, app, tenant, receipt_id
    );
    ...
    let rec_new = ReceiptNew {
        ...
        tenant_id,
        issuer_id: None,
        created_by_user_id: auth.user_id.as_ref().map(|_| Uuid::new_v4()),
        cid: input.cid,
        did: input.did,
        url: url.clone(),
    ...
    let rec = upsert_receipt(&st.pool, rec_new)
        .await
        .map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn receipts_get(
    ...
    let tenant_id: Uuid =
        sqlx::query_scalar("SELECT id FROM tenant WHERE app_id = $1 AND slug = $2")
            ...
            .await
            .map_err(|e| (axum::http::StatusCode::NOT_FOUND, e.to_string()))?;
    let rid = Uuid::parse_str(&id)
        .map_err(|_| (axum::http::StatusCode::BAD_REQUEST, "invalid id".into()))?;
    ...
    if let Some((cid, did, url, body, decision)) = row {
        Ok(Json(
            serde_json::json!({ "id": id, "cid": cid, "did": did, "url": url, "body": body, "decision": decision }),
        ))
    } else {
...
async fn keys_get(
    ...
    let tenant_id: Uuid =
        sqlx::query_scalar("SELECT id FROM tenant WHERE app_id = $1 AND slug = $2")
            ...
            .await
            .map_err(|e| (axum::http::StatusCode::NOT_FOUND, e.to_string()))?;
    let jwks: Option<serde_json::Value> = sqlx::query_scalar(
        "SELECT jwks FROM issuer WHERE tenant_id = $1 AND did = $2 AND active = TRUE",
    )
    .bind(tenant_id)
    .bind(&did)
    .fetch_optional(&st.pool)

...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Bundle { receipt, context, out } => { 
            println!("(stub) would bundle receipt={receipt} context={:?} out={out}", context);
            Ok(())
        }
...
fn cmd_decode(args: DecodeCmd) -> Result<()> {
    ...
    let v = nrf_core::decode(&bytes)
        .map_err(|e| anyhow::anyhow!("NRF decode error: {:?}", e))?;
